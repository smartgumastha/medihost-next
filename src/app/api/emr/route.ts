...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error("[GET /api/emr] Error:", err instanceof Error ? err.message : err);
...
export async function POST(request: NextRequest) {
  try {
    ...
    var res = await hmsFetch(`${BACKEND}/api/hospitals/${auth.hospitalId}/patients/${patientId}/visits`, auth, {
    ...
    console.error("[POST /api/emr] Error:", err instanceof Error ? err.message : err);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
...
export async function PUT(request: NextRequest) {
  try {
    ...
    console.error("[PUT /api/emr] Error:", err instanceof Error ? err.message : err);
    return NextResponse.json({ success: false, error: "Internal server error" }, { status: 500 });
  }
}
