...
export async function GET(request: NextRequest) {
  try {
    ...
    var url = `${BACKEND}/api/hospitals/${auth.hospitalId}/reports/collection-v2${searchParams ? "?" + searchParams : ""}`;
    ...
    console.error("[GET /api/reports/collection] Error:", err instanceof Error ? err.message : err);
