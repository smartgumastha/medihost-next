...
const BENEFITS = [
  { amount: '10%', label: 'ongoing monthly commission on every paying clinic you refer, forever', icon: '💰' },
  { amount: 'Up to 20%', label: 'bring 50+ clients, earn 20% ongoing', icon: '🚀' },
  { amount: 'Zero Investment', label: 'free to join, no targets, no pressure', icon: '🎯' },
];
...
export function ResellerRegistration() {
  ...
  const copyCode = () => {
    if (!result) return;
    navigator.clipboard.writeText(result.ref_code);
    setCopiedCode(true);
  ...
  const shareWhatsApp = () => {
    const msg = encodeURIComponent(
      `Hi! I'm inviting you to try MediHost - free clinic website + HMS software. Sign up here: https://${refLink}`
    );
  ...
  const earnings = Math.round(clinics * 999 * 0.1);
  ...
  if (result) {
    return (
      <div className="max-w-lg mx-auto px-4 py-16 text-center">
        ...
        <h2 className="text-2xl font-bold text-white mb-2">Welcome to MediHost Reseller Program!</h2>
        <p className="text-slate-400 mb-8">Share your link with doctors. Every signup earns you commission.</p>

        {/* Referral code */}
        <p className="text-sm text-slate-500 mb-2">Your referral code</p>
        <div className="flex items-center justify-center gap-3 bg-emerald-500/10 border border-emerald-500/30 rounded-xl px-6 py-4 mb-6">
        ...
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-lg px-4 py-3 mb-8">
        ...
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-green-600 hover:bg-green-700 text-white font-medium transition-colors mb-4"
        >
        ...
        <a
          ...
          className="inline-block w-full py-3 rounded-lg bg-gradient-to-r from-emerald-500 to-teal-600 text-white font-semibold hover:from-emerald-600 hover:to-teal-700 transition-all text-center"
        >
    ...
    <div className="max-w-3xl mx-auto px-4 py-12">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-slate-400">
          Refer clinics. Earn recurring commissions. No investment needed.
        </p>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-2xl p-8 mb-8">
        <h2 className="text-xl font-semibold text-white mb-1 text-center">How much can you earn?</h2>
        <p className="text-sm text-slate-500 text-center mb-6">Drag the slider to see your potential passive income</p>

        <div className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm text-slate-400">Clinics you refer per month</label>
            <span className="text-lg font-bold text-emerald-400">{clinics}</span>
          </div>
          <input
            type="range"
            min={1}
            ...
            className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer accent-emerald-500 [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-emerald-500 [&::-webkit-slider-thumb]:shadow-lg [&::-webkit-slider-thumb]:shadow-emerald-500/30"
          />
        ...
        <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-xl p-5 text-center">
          <p className="text-sm text-slate-400 mb-1">Your monthly passive income</p>
          <p className="text-4xl font-extrabold text-emerald-400">
            ₹{earnings.toLocaleString('en-IN')}
            <span className="text-base font-normal text-slate-500">/month</span>
          </p>
          <p className="text-xs text-slate-500 mt-2">
            {clinics} clinics × ₹999 avg plan × 10% commission
          </p>
        </div>
      </div>
